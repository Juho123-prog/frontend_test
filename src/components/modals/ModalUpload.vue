<template>
  <ModalLayout>
    ...
    <template v-slot:buttons>
      <button type="button" class="vf-btn vf-btn-primary" :disabled="uploading" @click.prevent="upload" style="background-color: #ff4b2b; width: 100px; height: 30px; padding-top: 5px;">
        {{ t('Upload') }}
      </button>
      <button type="button" class="vf-btn vf-btn-secondary" v-if="uploading" @click.prevent="cancel">{{ t('Cancel') }}</button>
      <button type="button" class="vf-btn vf-btn-secondary" v-else @click.prevent="close" style="padding-top: 5px; margin-right: 10px;">{{ t('Close') }}</button>
    </template>
  </ModalLayout>
</template>
